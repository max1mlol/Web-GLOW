...
//  4. Card-нуудыг зурна        (components.js)
...
function renderActiveFilterBadges(params) {
  ...
  const filterMap = {
    ...
    q: params.q ? `"${params.q}"` : '',
    minPrice: params.minPrice ? `$${params.minPrice}+` : '',   // жишээ: "$20+" (доод хязгаар)
    maxPrice: params.maxPrice ? `до $${params.maxPrice}` : '', // жишээ: "до $80" (дээд хязгаар)
  };
  ...
  Object.entries(filterMap).forEach(([key, label]) => {
    ...
    const badge = ActiveFilterBadge(label, () => {
      ...
      // Үнийн мужийн нэг талыг хасахад нөгөө талыг нь ч цэвэрлэнэ —
      // учир нь хэсэгчилсэн үнийн муж утгагүй байна.
...
function renderProducts(products) {
  ...
  // Card бүрийг ProductCard component-оор үүсгэнэ
